...
from teco.w4c_data_utils import *
...
class RainData(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        self.preprocess_target = preprocess_HRIT # NOTE: changed from preprocess_OPERA
    ...
    def load_in(self, in_seq, seq_r, metadata, loaded_input=False):
        in0 = time.time()
        # NOTE: this has a different preprocessing step compared to load_out by default
    ...
    def load_out(self, out_seq, seq_r, metadata):
        ...
        if self.data_split not in ["test", "heldout"]:
            output_data, out_masks = get_sequence(
                ...
                self.input_product,
                self.sat_bands,
                self.preprocess_input, # NOTE: changed
                self.swap_time_ch,
                self.out_ds,
            )
